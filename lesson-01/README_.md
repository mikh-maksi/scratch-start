...
4. Блок команд.   
5. Поле команд (поле для составление кодов).   
6. Рабочее Поле (поле фонов и персонажей).   
7. Блок настройки спрайтов.   
8. Блок управления сценами.   

## Типы команд
### Движение
* **идти 10 шагов** - перемещает персонаджа на 10 шагов вперед
* **повернуть на 15 градусов** - поворачивает персонажа на 15 градусов (вправо или влево)
* **Перейти на случайное положение** - перемещает персонажа в случайное положение поля
* **Перейти на x:0 y:0** - перемещает персонажа в точку с указаной координатой (в примере 0;0)
* **Плыть 1 секунд к случайное положение** - персонаж плывет в случайную точку в течении указанного числа секунд (в примере 1). 
* **Плыть 1 секунд в точку x:0 y:0** - перемещает плывет в точку с указаной координатой (в примере 0;0) в течении указанного числа секунд (в примере 1).   
Команда "перейти" - перемещает в точку мгновенно (переход не виден), команда "плыть" перемещает в точку плавно (переход виден), поэтому команду "плыть" удобно использовать для программирования простой анимации перемещения.
